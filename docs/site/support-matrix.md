...
| --- | --- | --- | --- | --- |
| `stripe` | `workflow-compatible` | checkout sessions, payment intents, customers, products, prices, subscriptions, invoices, refunds, fake webhook sender | success, failure, auth error, rate limit, timeout | SDK evidence `stripe@22.3.0` passes for supported local workflows; fake state, list/retrieve, validation errors, and idempotency-key replay are available. Known gaps: no fraud, billing network, tax, disputes, Connect, or full Billing lifecycle. |
