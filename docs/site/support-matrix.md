...
| --- | --- | --- | --- | --- |
| `stripe` | `workflow-compatible` | checkout sessions, payment intents, customers, products, prices, subscriptions, invoices, refunds, fake webhook sender | success, failure, auth error, rate limit, timeout | SDK evidence `stripe@22.2.0` passes for supported local workflows; fake state, list/retrieve, validation errors, and idempotency-key replay are available. Known gaps: no fraud, billing network, tax, disputes, Connect, or full Billing lifecycle. |
| `openai` | `workflow-compatible` | models, chat completions, responses, embeddings, files, batches | chat success, SSE stream success, rate limit, context length, auth error | SDK evidence `openai@6.42.0` passes for supported local workflows; deterministic fake inference, stateful response lookup, streaming, embeddings, files, and batches are available. Known gaps: no real model quality, tokenization parity, hosted tools, vector stores, or provider scheduling. |
...
| `zoho-oauth` | `workflow-compatible` | authorize redirect, access token exchange, user info | oauth success, invalid code, invalid token | Minimal Zoho OAuth2 authorization-code workflow: immediate authorize redirect with `state` echo, token exchange, and user info using the `Zoho-oauthtoken` auth scheme. Fake codes are one-time and client-bound; returned `Email`/`Display_Name` are configurable via env or authorize query. Known gaps: no real Zoho login UI, MFA, data-center/org routing, token refresh, scope enforcement, or full user profile fields. |
